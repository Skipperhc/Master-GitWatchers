package com.pp5.apiWeb.controllers;
...
public class GitHubController {
	...
	// public ModelAndView resultados(List<PorcentagemLinguagens>
	// porcentagensLinguagem) {
	//
	// ModelAndView mav = new ModelAndView("resultados");
	//
	// mav.addObject("repositos", porcentagensLinguagem);
	// /* mav.addObject("repositorios", ""); */
	//
	// return mav;
	// }

	@GetMapping("/{user}/porcentagens")
	public ModelAndView testar(@PathVariable String user) {
		...
		for (Repositorio repos : repositorios) {
			if (repos.getLinguagem() == null) {
				...
				if (percent != null) {
					...
					mav.addObject("repositorios", porcentagensLinguagem);
	...
	// @GetMapping("/{user}/porcentagens")
	// public List<PorcentagemLinguagens> listarPorcentagens(@PathVariable
	// String user) {
	//
	// RestTemplate template = new RestTemplate();
	//
	...
	//
	// ResponseEntity<Repositorio[]> response =
	// template.getForEntity(uri.toUriString(), Repositorio[].class);
	// Repositorio[] listaRepositorioitorios = response.getBody();
	//
	// List<Repositorio> repositorios = Arrays.asList(listaRepositorioitorios);
	//
	...
	// for (Repositorio repos : repositorios) {
	// if (repos.getLinguagem() == null) {
	// if (!porcentagensLinguagem.stream().anyMatch(x ->
	// x.getLinguagem().equals("Sem linguagem"))) {
	// porcentagensLinguagem.add(new PorcentagemLinguagens("Sem linguagem", 1));
	// } else {
	// PorcentagemLinguagens percent = porcentagensLinguagem.stream()
	// .filter(x -> x.getLinguagem().equals("Sem
	// linguagem")).findFirst().orElse(null);
	// if (percent != null) {
	//
	// int index = porcentagensLinguagem.indexOf(percent);
	// percent.setQtd(percent.getQtd() + 1);
	...
	// porcentagensLinguagem.add(new PorcentagemLinguagens(repos.getLinguagem(),
	// 1));
	// } else {
	//
	// PorcentagemLinguagens percent = porcentagensLinguagem.stream()
	//
	// .filter(x ->
	// x.getLinguagem().equals(repos.getLinguagem())).findFirst().orElse(null);
	// if (percent != null) {
	//
	// int index = porcentagensLinguagem.indexOf(percent);
	// percent.setQtd(percent.getQtd() + 1);
	...
	public List<UsuarioContribuicao> listarContribuicoes(
